...
struct MyView: View {
    ...
    private var initialOffsetY: CGFloat { screenHeight}
    ...
    var body: some View {
        VStack(spacing: 0) {
//            Spacer()
//                .background(Color.navPurpleBlueGradient)
//                .frame(maxWidth: .infinity)
//                .frame(height: 100)
            // 이미지 상단 0~50px 영역만 표시
...
//                .background(Color.purpleBlueGradient.ignoresSafeArea(edges: .top))
                //            BottomDragSheet()
                // 단일 ScrollView:
